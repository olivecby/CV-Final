...
R2Point R2Image::
detectFeature(R2Image *featureImage, R2Point prevPoint) {


	// get size of the feature image
	int w = featureImage->width;
	int h = featureImage->height;
	...
	rangeStartX = prevX - 20;
	rangeStartY = prevY - 20;
	rangeEndX = prevX + 20;
	rangeEndY = prevY + 20;
	...
	R2Point minLocation;
	R2Pixel minSum = R2Pixel(10000,10000,10000,10000);
	...
	for (int ly = rangeStartY; ly <= rangeEndY ; ly=ly+2) {
		for (int lx =  rangeStartX; lx <= rangeEndX; lx=lx+2) { 

			R2Pixel sum;
			for(int y=0; y<h; y=y+1){
				for(int x=0; x<w; x=x+1){

					R2Pixel diff = Pixel(lx+x, ly+y) - featureImage->Pixel(x,y);
					R2Pixel sqDiff = diff * diff;
					sum+= sqDiff;
				}
			}

			if(minSum[0]+minSum[1]+minSum[2] > sum[0]+sum[1]+sum[2] ){
				//printf("%f,%f,%f\n",sum[0],sum[1],sum[2]);
				minSum = sum;
				minLocation = R2Point(lx,ly);
			}

			if(sum[0]<1 && sum[1]<1 && sum[2]<1) break;
			
		}

	}

	minLocation.SetX(minLocation.X()+w/2);
	minLocation.SetY(minLocation.Y()+h/2);

	for(int i=0; i<5; i++){
		for(int j=0; j<5; j++){
			SetPixel(minLocation.X()+i, minLocation.Y()+j,R2Pixel(0,1,0,0));
		}
	}
...
void R2Image::
replaceFrameContent(double* xcoords, double* ycoords, R2Image* replacementImage)
{
	// estimate H
	double** matrixH = dmatrix(1, 3, 1, 3);
	matrixH = svdTest(xcoords, ycoords, 4);
	...
	R2Point lowerLeft (xcoords[0], ycoords[0]);
	R2Point lowerRight (xcoords[6], ycoords[6]);
	R2Point upperLeft (xcoords[2], ycoords[2]);
	R2Point upperRight (xcoords[4], ycoords[4]);

	/*printf("lowerLeft: (%g, %g)\n", lowerLeft.X(), lowerLeft.Y());
	printf("lowerRight: (%g, %g)\n", lowerRight.X(), lowerRight.Y());
	printf("upperLeft: (%g, %g)\n", upperLeft.X(), upperLeft.Y());
	printf("upperRight: (%g, %g)\n\n", upperRight.X(), upperRight.Y());*/


	
	double corners_x[4] = { xcoords[0], xcoords[2], xcoords[4] ,xcoords[6] };
	double corners_y[4] = { ycoords[0], ycoords[2], ycoords[4] ,ycoords[6] };


	// find max & min of x- and y-coords. these would be the leftmost, rightmost, top, bottom
	double top = *std::max_element(std::begin(corners_y), std::end(corners_y));
	double bottom = *std::min_element(std::begin(corners_y), std::end(corners_y));
	double leftmost = *std::min_element(std::begin(corners_x), std::end(corners_x));
	double rightmost = *std::max_element(std::begin(corners_x), std::end(corners_x));
	...
	*/
	...
	for (int y = bottom; y <= top; y++) {
		for (int x = leftmost; x <= rightmost; x++) {
			...
			// transformed point is the point remapped from the replacement image to the 4-cornered frame
			R2Point trans_point = matrixMult(x, y, matrixH);
			double trans_x = trans_point.X();
			double trans_y = trans_point.Y();

			// loop through all pixels in the warped image and take color samples from the original image 
			if (inFrame(R2Point(x, y), upperLeft, upperRight, lowerRight, lowerLeft)) {
				if (trans_x < w && trans_y < h && trans_x > 0 && trans_y > 0) {
					R2Pixel value = (replacementImage->Pixel(trans_x, trans_y));
					SetPixel(x, y, value);
					/*else {
					printf("not in frame \n");
					}*/
...
bool R2Image:: 
inFrame(R2Point pt, R2Point v1, R2Point v2, R2Point v3, R2Point v4) {
	return PointInTriangle(pt, v1, v2, v4) || PointInTriangle(pt, v2, v4, v3);
}

R2Point R2Image:: 
matrixMult(double x, double y, double** H)
{
	double newX = H[1][1] * x + H[1][2] * y + H[1][3];
	double newY = H[2][1] * x + H[2][2] * y + H[2][3];
	double newZ = H[3][1] * x + H[3][2] * y + H[3][3];

	return R2Point(newX / newZ, newY / newZ);
}
 
float R2Image:: 
sign(R2Point p1, R2Point p2, R2Point p3)
{
	return (p1.X() - p3.X()) * (p2.Y() - p3.Y()) - (p2.X() - p3.X()) * (p1.Y() - p3.Y());
}

bool R2Image::
PointInTriangle(R2Point pt, R2Point v1, R2Point v2, R2Point v3)
{
	bool b1, b2, b3;
