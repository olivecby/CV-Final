...
class R2Image {
 public:
  ...
  // show how SVD works
  double** svdTest(double *xcoords, double* ycoords, int size);
  ...
  R2Point detectFeature(R2Image *featureImage, R2Point prevPoint);
  void replaceFrameContent(double* xcoords, double* ycoords, R2Image* replacementImage);

  // Supporting functions
  void line(int x0, int x1, int y0, int y1, float r, float g, float b);
  R2Point matrixMult(double x, double y, double** H);
  bool inFrame(R2Point pt, R2Point v1, R2Point v2, R2Point v3, R2Point v4);
  float sign(R2Point p1, R2Point p2, R2Point p3);
  bool PointInTriangle(R2Point pt, R2Point v1, R2Point v2, R2Point v3);
