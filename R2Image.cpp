		...
		for (int x = edgex; x < width - edgex; x++) {
			for (int fx = 0; fx < window_width; fx++) {
				for (int fy = 0; fy < window_height; fy++) {
					window.push_back(Pixel(x + fx - edgex, y + fy - edgey));
				}
			}
		
			// std::sort(window.begin(), window.end());		-- no matching overloaded function error!!
			final.SetPixel(x, y, window[window_width * window_height / 2]);
		}
	}
	*this = final;
}
		...
		for (int x = 2; x < width - 2; x++) {
			...
			for (int i = 0; i < 5; i++) {
				for (int j = 0; j < 5; j++) {
					...
					R2Pixel p = Pixel(neigh_x, neigh_y) - Pixel(x, y);
					double pixelValue = p.Red() + p.Blue() + p.Green();

					double gi = Gaussian(pixelValue, sigmaI);
					double gs = Gaussian(distance(x, y, neigh_x, neigh_y), sigmaS);
